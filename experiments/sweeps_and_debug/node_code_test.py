...
def generate_node_codes(nodes_per_community: int, num_communities: int,
                        seed: int) -> Dict:
    ...
    codes = []
    while len(codes) < N:
        c = "".join(rng.choice(CODE_CHARS) for _ in range(4))
    ...
    for rank_in_perm, node_id in enumerate(perm):
        ...
        code_map[node_id] = codes[rank_in_perm]
        nodes_by_comm[comm].append(node_id)

    print(f"  Nodes: {N} total, {nodes_per_community}/community, {C} communities")
    print(f"  Codes: {len(codes)} unique 4-char codes assigned (random permutation)")
    return {
        "N": N,
...
def generate_qa(graph: Dict, eval_frac: float, rng: random.Random) -> tuple:
    code_map = graph["code_map"]
    ...
    for comm_id, nodes in enumerate(nodes_by_comm):
        ...
        for node_id in nodes_shuf[:n_eval]:
            eval_qa.append({
                "node": node_id,
                "query": f"What is the identifier of node_{node_id} in graph G?",
                "answer": f"The identifier of node_{node_id} is {code_map[node_id]}.",
                "code": code_map[node_id],
                "community": comm_id,
            })
        for node_id in nodes_shuf[n_eval:]:
            train_qa.append({
...
def evaluate(model, router, moe_layers, eval_qa, tokenizer, device, arch,
             batch_size=16, max_new=16) -> tuple:
    model.eval()
    ...
    for i in range(0, len(eval_qa), batch_size):
        ...
        for j, rec in enumerate(batch_recs):
            gen_text = tokenizer.decode(gen[j, plen:], skip_special_tokens=True).strip()
            if rec["code"] in gen_text:
                correct += 1
            total += 1
...
def run_one(nodes_per_comm: int, arch: str, tokenizer, cfg: Dict,
    ...
    graph = generate_node_codes(nodes_per_comm, C, cfg["seed"])
    train_qa, eval_qa = generate_qa(graph, cfg["eval_frac"], rng)
    ...
    total_steps = min(max_steps, steps_per_epoch * 10)
    total_steps = max(total_steps, min(max_steps, 500))
...
def parse_args():
    ...
    p.add_argument("--output_dir", default="outputs/node_code_test")
...
def main():
    ...
    print(f"[node_code] device={cfg['device']}, sizes={args.sizes}, archs={args.archs}")
    print(f"[node_code] BASE_RANK={cfg['base_rank']} (EQUAL for both architectures)")
    print(f"[node_code] MoE: {cfg['num_communities']} experts × rank={cfg['base_rank']}")
    print(f"[node_code] Single: rank={cfg['base_rank']} (same per-expert capacity)")
    print(f"[node_code] Task: 4-char unique code per node  (0 % trivial baseline)")
    print()
    ...
    for size in args.sizes:
        for arch in args.archs:
            run_dir = os.path.join(args.output_dir, f"n{size * cfg['num_communities']}_{arch}")
            try:
                ...
                print(f"[node_code] FAILED n={size * cfg['num_communities']} {arch}: {e}")
                import traceback; traceback.print_exc()

    print(f"\n{'='*70}")
    print("  NODE-CODE MEMORISATION TEST  (rank_single = rank_local = 4)")
    print(f"{'='*70}")
    print(f"  {'Total N':>8} | {'single EM':>10} | {'MoE EM':>8} | {'Δ(MoE-single)':>14}")
    print(f"  {'-'*8}-+-{'-'*10}-+-{'-'*8}-+-{'-'*14}")
    by_n: Dict[int, Dict] = {}
    for r in results:
        key = r["total_nodes"]
    ...
    for n in sorted(by_n):
        ...
        d = m - s
        marker = " ← MoE WINS" if d > 0.02 else (" ← single wins" if d < -0.02 else "  ≈ tie")
        print(f"  {n:>8} | {s:>10.3f} | {m:>8.3f} | {d:>+14.3f}{marker}")
    print()
